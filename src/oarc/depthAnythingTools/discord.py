...
def virtual_webcam_stream(encoder):
    """Stream depth display to virtual webcam"""
    cap = cv2.VideoCapture(0)
    with pyvirtualcam.Camera(width=640, height=480, fps=30, fmt=PixelFormat.BGR, backend='obs') as cam:
...
def start_virtual_webcam(model_name):
    ...
    virtual_webcam_stream(encoder)

# Create Gradio interface
with gr.Blocks() as demo:
    gr.Markdown("# Depth Anything V2 Virtual Webcam for Discord")
